...
MeshWifiInterfaceMac::SetWifiPhy (Ptr<WifiPhy> phy)
{
  ...
  //added by jason
  m_beaconDca->SetPhy (m_phy);

  //iterate through all the queue types (may need to change this so we don't affect the priority and other types of traffic)
  for (Queues::const_iterator i = m_queues.begin (); i != m_queues.end (); i++)
      i->second->SetPhy(m_phy);
}
Ptr<WifiPhy>
