...
#define B1			2.0
#define B2			5.0
#define EPCHANGEFACTOR	0.01					/* how much to adjust alpha up or down by */
#define EPPACKETTRIAL	50
...
WifiMacQueue::Enqueue (Ptr<const Packet> packet, const WifiMacHeader &hdr)
{
  ...
  if(MB)
  {
	...
	{
		R = (eps.ALPHA / pow(hops,eps.BETA1)) + ((1-eps.ALPHA) / (pow(hops,eps.BETA2)));
		R = R * 5;
		if(hops == 1)
			R = 0.95;
		//std::cout << "HOPS: " << hops << " R: " << R << std::endl;	
	    ...
	    {			
			if(packetCount > EPPACKETTRIAL)
			{
				...
				double pdrscore = pdr - eps.previousPDR;											//if pdr goes up, this will be higher
				double delayscore = -1*((delay - eps.previousAvgDelay) / eps.previousAvgDelay);		//if delay goes down this will be higher
				if(delayscore > 1)
					delayscore = 1;
				if(delayscore < -1)
					delayscore = -1;
				eps.previousAvgDelay = delay;
				eps.previousPDR = pdr;
				double score = 0.5*pdrscore + 0.5*delayscore;
				
				int action = ep.predict(score);
				
				if(action == 0)
				{
					eps.ALPHA = eps.ALPHA - EPCHANGEFACTOR;
					//cout << "DOWN: " << eps.ALPHA << endl;
				}
				else
				{
					eps.ALPHA = eps.ALPHA + EPCHANGEFACTOR;
					//cout << "UP: " << eps.ALPHA << endl;;
				}
